...
export default function Header({
  ...
  setIsUncontrolledOpen: React.Dispatch<React.SetStateAction<boolean>>;
  setIsRHFOpen: React.Dispatch<React.SetStateAction<boolean>>;
}) {
  return (
    <header className="sticky top-0 border-b border-accent-dark/20 p-2">
      <div className="flex flex-row items-center justify-between gap-2 max-w-6xl mx-auto w-full">
        <svg
          fill="#000000"
          version="1.1"
          id="Capa_1"
        ...
        <div className="flex flex-row flex-wrap justify-end items-center gap-2">
          ...
          <Button onClick={() => setIsRHFOpen(true)}>React Hook Form</Button>
        </div>
      </div>
    </header>
  );
