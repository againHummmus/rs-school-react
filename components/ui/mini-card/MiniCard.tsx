...
export default function MiniCard({
  ...
  return (
    <Link
      href={{ pathname: '/', query: { details: String(mal_id) } }}
      prefetch={false}
    ...
    >
      ...
      <div className="flex flex-col min-w-0">
        <p className="text-background font-medium text-sm leading-tight truncate">
          {title}
        </p>
        {year ? (
          <p className="text-background/60 text-xs mt-0.5">{year}</p>
        ) : null}
      </div>
    </Link>
  );
